...
def get_linear_lr_func(lr_init, lr_final, start_decay_step, max_steps):
    """
    创建一个学习率函数，该函数在开始时保持学习率不变，在达到特定步骤后线性衰减至最终学习率。

    :param lr_init: float, 初始学习率。
    :param lr_final: float, 最终学习率。
    :param start_decay_step: int, 开始衰减学习率的步骤。
    :param max_steps: int, 总的迭代次数。
    :return: 函数，该函数接受训练步骤作为输入并返回对应的学习率。
    """
    
    def helper(step):
        if step < 0:
            return 0.0  # 如果步数小于0，直接返回0.0，不进行训练
        if step < start_decay_step:
            return lr_init  # 如果当前步数小于开始衰减的步数，保持初始学习率不变
        elif step <= max_steps:
            # 从开始衰减的步骤到最大步骤之间线性衰减学习率
            return lr_final + (lr_init - lr_final) * (max_steps - step) / (max_steps - start_decay_step)
        else:
            return lr_final  # 超过最大步骤后，学习率保持在最终学习率
