...
def distillation(global_model: GaussianModel,
    ...
    with torch.no_grad():
        ...
        for i in range(len(viewmats)):
            #从local model 渲染local camera视角的图片，相当于渲染一个真值（teacher model）
        ...
        #找到global 视角下能看到local model的点的相机, 根据能看到点的多少加权采样
        ...
        #将global model中可见的点的透明度重置 
        # we reset the opacity of Gaussians in the local model and the Gaussians of the global model around those of the local model before distillation.
